...
static int next_len(ASN1Parser *parser)
{
    ...
    if (parser->state->remaining < 2)
        return 0;
    read_length(parser->state->buffer + 1, &len);
    return len;
}
...
static void *next_payload(ASN1Parser *parser)
{
    ...
    if (parser->state->remaining < 2)
        return 0;
    p = read_length(parser->state->buffer + 1, &len);
    return p;
}

static void push_state(ASN1Parser *parser, void *buffer, int remaining)
{
    ASN1ParserState *state = malloc(sizeof(ASN1ParserState));
    state->buffer = buffer;
...
static void consume(ASN1Parser *parser)
{
    ...
    parser->state->remaining -= len + payload - parser->state->buffer;
    parser->state->buffer = payload + len;
}

ASN1Parser *asn1_create_parser(void *buffer, int len)
{
    ASN1Parser *parser = malloc(sizeof(ASN1Parser));
    parser->state = NULL;
...
int asn1_parse_structure(ASN1Parser *parser, int *type)
{
    ...
    *type = t;
    ...
    int len = next_len(parser);
    void *payload = next_payload(parser);
    if (payload + len > parser->state->buffer + parser->state->remaining)
        return 0;
    consume(parser);
